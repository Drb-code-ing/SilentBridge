...
1. **小切口，深闭环**：先跑通“出示 → 收听 → 理解 → 保存”，而不是空做大平台。  
...
2. 点 **医院问诊**  
3. 出示开场白（可点全屏）  
4. 开始收听；不便用麦克风时可用 **演示字幕** 或手动输入  
5. 查看 **已确认 / 待确认 / 风险**  
6. 可 **保存这次重点**，或 **重新收听 / 改文字再整理**  
7. 到「记录」页刷新，确认摘要仍在  

也可直接点底部「沟通」体验 **通用沟通**；政务 / 药店 / 问路可验证不同场景重点。
...
1. **分阶段推进**：每个阶段只做一件可验收的事，例如先让字幕逐条出现。  
2. **生成可运行代码**：目录结构、组件、样式、状态流，到 typecheck / build 在会话中闭环。  
3. **阶段留证**：过程截图 + Session ID + 完成摘要，避免最后补材料。  
...
感谢评审体验。欢迎点开链接走一遍 60 秒路径，也欢迎对无障碍沟通场景提出建议。
