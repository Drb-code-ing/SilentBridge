    ...
    <Card className="p-4 sm:p-5" variant="solid">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <h3 className="text-2xl font-black text-neutral-950">我听见了</h3>
        </div>
      ...
      <div className="mb-4 grid grid-cols-[1fr_auto] gap-2">
        <Button onClick={onStartSimulation} disabled={allLines.length === 0} size="md">
          {isSimulating ? "接收中" : isComplete ? "重新模拟" : "开始模拟"}
        </Button>
      ...
      <div className="max-h-[26rem] space-y-3 overflow-y-auto pr-1">
        {lines.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-neutral-300 bg-white/62 p-6 text-center">
            <p className="text-lg font-black text-neutral-900">等待字幕输入</p>
            <p className="mt-2 text-sm leading-6 text-neutral-600">
              点击开始模拟后，对话会逐句进入。重点句会自动高亮，适合在嘈杂场景中快速抓住信息。
            </p>
          </div>
        ) : (
