const SCENE_CONTEXT: Record<string, string> = {
  clinic: "医院问诊场景。重点关注：诊断判断、用药剂量天数、复诊时间、急诊红线（胸闷/持续高烧等）。不做诊断建议，只整理医生已说内容。",
  pharmacy: "药店/用药场景。重点关注：药名、用量、服药时间、禁忌、症状、不适处理。",
...
const SCENE_EXAMPLE: Record<string, string> = {
  clinic: `示例：
输入："这次主要是咽喉炎，先按这个剂量连续吃三天。如果出现胸闷、持续高烧，不能等复诊，要马上来急诊。三天后到门诊复诊。"
输出：
{
  "confirmed": ["判断：咽喉炎", "用药：连续吃三天", "复诊：三天后门诊", "急诊红线：胸闷或持续高烧马上去急诊"],
  "missing": ["药名和每天几次还没写清"],
  "risks": [{"level": "high", "text": "胸闷或持续高烧不能等复诊，应立即急诊"}],
  "suggestedQuestion": "请再写清楚药名、每天几次，以及什么情况要马上去急诊。",
  "plainSummary": "医生判断为咽喉炎，需连续服药三天并复诊；若胸闷或持续高烧应立即急诊。",
  "correctedText": "这次主要是咽喉炎，先按这个剂量连续吃三天。如果出现胸闷、持续高烧，不能等复诊，要马上来急诊。三天后到门诊复诊。"
}`,
  pharmacy: `示例：
